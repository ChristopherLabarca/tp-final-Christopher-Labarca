 ...
 */
export const getImageForBreed = async (especie: string, raza: string): Promise<string> => {
  try {
 ...
 */
async function getDogImageByBreed(raza: string): Promise<string> {
  const maxRetries = 2;
 ...
 */
async function getCatImageByBreed(raza: string): Promise<string> {
  const maxRetries = 2;
  ...
  try {
    ...
    const genericData = await genericResponse.json() as any;
